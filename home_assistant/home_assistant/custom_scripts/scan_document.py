...
def format_body_content(output_type, resolution):
    ...
    post_body = post_body.replace('{{ resolution }}', resolution_values[resolution]['resolution'])
    post_body = post_body.replace('{{ width }}', resolution_values[resolution]['width'])
    post_body = post_body.replace('{{ height }}', resolution_values[resolution]['height'])
    post_body = post_body.replace('{{ format }}', format[output_type])
    post_body = post_body.replace('{{ content_type }}', content_type[output_type])
    return post_body
